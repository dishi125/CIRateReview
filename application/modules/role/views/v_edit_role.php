						...
						<div class="card">
							...
							<div class="card-body pt-0">
								<form method="post" id="edit_role">
									<input type="hidden" class="form-control" id="role_id" name="role_id" value="<?= $GetRoleWithPrivilegePermissionByRoleId[0]['role_id'] ?>">
									<div class="row">
										<div class="col-xxl-3 col-md-6">
											<div>
												<label for="labelInput" class="form-label">Role Name</label>
												<input type="text" class="form-control" id="role_name" name="role_name" value="<?= $GetRoleWithPrivilegePermissionByRoleId[0]['name'] ?>">
										...
										<div class="col-xxl-3 col-md-6">
											<div>
												...
												<input type="text" class="form-control" id="description" name="description" value="<?= $GetRoleWithPrivilegePermissionByRoleId[0]['description'] ?>">
											</div>
										</div>
									</div>
								</form>
								...
								<div class="pagination-list border p-4 mt-5">
									...
									<?php foreach ($GetRoleWithPrivilegePermissionByRoleId as $privilege) { ?>
										<form class="permissionForm" method="post">
											<div class="mx-n3 row border-bottom-inset p-2">
												<input type="hidden" value="<?= $privilege['privilege_id'] ?>" name="privilege_id">
												<input type="hidden" value="<?= $privilege['display_name'] ?>" name="display_name">
												<div class="col-4">
													<h5 class="fs-14 mb-1"><?= $privilege['display_name'] ?></h5>
												</div>
												<div class="col-2">
													<div class="flex-grow-1 form-check form-check-outline form-check-secondary"><input class="form-check-input" type="checkbox" name="is_add" value="true" <?= $privilege['is_add']==true ? 'checked' : ''  ?>></div>
												</div>
												<div class="col-2">
													<div class="flex-grow-1 form-check form-check-outline form-check-secondary"><input class="form-check-input" type="checkbox" name="is_edit" value="true" <?= $privilege['is_edit']==true ? 'checked' : ''  ?>></div>
												</div>
												<div class="col-2">
													<div class="flex-grow-1 form-check form-check-outline form-check-secondary"><input class="form-check-input" type="checkbox" name="is_delete" value="true" <?= $privilege['is_delete']==true ? 'checked' : ''  ?>></div>
												</div>
												<div class="col-2">
													<div class="flex-grow-1 form-check form-check-outline form-check-secondary"><input class="form-check-input" type="checkbox" name="is_view" value="true" <?= $privilege['is_view']==true ? 'checked' : ''  ?>></div>
												</div>
											</div>
										</form>
									<?php } ?>
...
<button type="button" id="success_toast" data-toast data-toast-text="Role Updated Successfully" data-toast-gravity="top" data-toast-position="right" data-toast-duration="3000" data-toast-close="close" class="btn btn-light w-xs" data-toast-classname="success"></button>
...
$(document).on('click', '#save_role', function (){
	...
	var formData = new FormData($('#edit_role')[0]);
