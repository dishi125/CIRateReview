					...
					<div class="col-xl-12 col-lg-12">
						<div class="card">
							...
							<div class="card-body pt-0">
								<form method="post" id="add_website">
								<div class="row">
...
$(document).on("click", "#save_website", function (){
	...
	var formData = new FormData($('#add_website')[0]);
	formData.append('type', '<?= $type ?>');

	if(validate_add_website_form()){
		$.ajax({
			...
			success: function (res) {
				if (res.status == 0){
					...
					$("#success_toast").attr('data-toast-text', "Website added successfully.");
...
function validate_add_website_form()
{
	$('#add_website').validate({
		rules: {
			website: {
				required: true,
			},	
