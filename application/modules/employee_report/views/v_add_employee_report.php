...
<head>
	...
	<title> Add Employee Report </title>
				...
				<div class="row">
					<div class="col-12">
						<div class="page-title-box d-sm-flex align-items-center justify-content-between">
							<h4 class="mb-sm-0">Add Employee Report</h4>
							<div class="page-title-right">
								<ol class="breadcrumb m-0">
									<li class="breadcrumb-item"><a href="<?= base_url('employee_report') ?>">Employee Report</a></li>
									<li class="breadcrumb-item active">Add Employee Report</li>
								</ol>
							</div>
						...
						<div class="card">
							<div class="card-header border-0">
								<div class="d-flex align-items-center">
									<h5 class="card-title mb-0 flex-grow-1">Add Employee Report</h5>
								</div>
							</div>

							<div class="card-body pt-0">
								<form method="post" id="add_employee_report" enctype="multipart/form-data">
									<div class="row">
...
<!--												<input type="time" class="form-control" id="break_hours" name="break_hours">-->
...
$(document).on('change','#attachments', function(event) {
	...
	for(var j=0; j<len; j++) {
		...
		/*} else if(mime_type[0] == "video") {
			src = "<?= base_url('assets/images/video.png') ?>";
		} else {
			src = "<?= base_url('assets/images/file_icon.png') ?>";
		}*/
...
$(document).on("click", "#save_employee_report", function() {
	$(this).attr('disabled', 'disabled');
	var formData = new FormData($('#add_employee_report')[0]);
	var userId = $("#users_dropdown").val();
	if (userId != '') {
		formData.append('customer', userId);
	}
	// var ins = document.querySelector("[name='images[]']").files.length;
	...
	if (validate_add_employee_report_form()) {
		$.ajax({
			...
			success: function(res) {
				if (res.status == "error") {
					...
					$("#success_toast").attr('data-toast-text', "Employee report added successfully.");
...
function validate_add_employee_report_form(){
	$('#add_employee_report').validate({
		rules: {
			start_time: {
				required: true,
			},
