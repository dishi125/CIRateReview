				...
				<div class="row">
					<div class="col-12">
						<div class="page-title-box d-sm-flex align-items-center justify-content-between">
							<h4 class="mb-sm-0" id="page_title">View Cities</h4>
				...
				<div class="row">
					<div class="col-12">
						<div class="flex-shrink-0">
							<button type="button" class="btn btn-primary waves-effect waves-light list_btn" id="hotel_list_btn" website-db="<?= $website_db ?>">Get Hotel List</button>
						</div>
						...
						<div class="card mt-1">
							...
							<div class="card-body">
								<div class="live-preview">
									<div class="col-8">
										<div class="table-responsive" id="view_all_cities_div">
											<table class="table table-bordered align-middle table-nowrap mb-0" id="view_all_cities_table">
												<thead>
												<tr>
													<th style="width: 42px;"></th>
													<th><input type="text" placeholder="State" class="form-control" name="search_state_cities_table" id="search_state_cities_table" website-db="<?= $website_db ?>"></th>
													<th><input type="text" placeholder="City" class="form-control" name="search_city_cities_table" id="search_city_cities_table" website-db="<?= $website_db ?>"></th>
												</tr>
												<tr>
													<th style="width: 42px;"></th>
													<th>State</th>
													...
													<tr>
														<td>
															<div class="form-check">
																<input class="form-check-input" type="checkbox" value="<?= $city['city_id'] ?>" id="responsivetableCheck" name="cities[]">
											...
											<div id='view_all_city_pagination' style="float: right" class="mt-2 pagination_link"><?= $pagination ?></div>
										</div>
									</div>
								</div>
							</div>
...
var cities;
$(document).on('click', '#view_all_city_pagination a', function(e) {
	e.preventDefault();
	var pageno = $(this).attr('data-ci-pagination-page');
	city_table_data(pageno);
});
...
function city_table_data(pageno){
	...
	$.ajax({
		...
		method: "POST",
		data: {"search_state": search_state, "search_city": search_city},
		beforeSend: function(jqXHR){
			xhrPool.push(jqXHR);
			$("#view_all_cities_div").find('.loading-ct').show();
		},
		success: function(res) {
			var res = JSON.parse(res);
			if (res.status == 1) {
				var html = city_table_html(res.cities, false);
				$("#view_all_cities_table tbody").html(html);
				$("#total_records").html("Total No. of records " + res.total_records);
				$("#view_all_city_pagination").html(res.pagination);
		...
		complete: function (){
			$("#view_all_cities_div").find('.loading-ct').hide();
		},
		error: function (){
			$("#view_all_cities_div").find('.loading-ct').hide();
		}
	...
	$.ajax({
		...
		success: function(res) {
			...
			if (res.status == 1) {
				...
				if (need_update_full_table) {
					$("#view_all_cities_table").html(html);
				}
				else {
					$("#view_all_cities_table tbody").html(html);
				}
	...
	$.ajax({
		...
		success: function(res) {
			...
			if (res.status == 1) {
				...
				$("#total_records").html("Total No. of records " + res.total_records);
				$(".pagination_link").html(res.pagination);
				$(".pagination_link").attr("id", "view_all_city_pagination");
				$(".list_btn").attr('id', 'hotel_list_btn');
				$(".list_btn").html("Get Hotel List");
				var breadcrumb_link = `<li class="breadcrumb-item"><a href="<?= base_url('system_analysis_dashboard') ?>">System Analysis Dashboard</a></li>
									<li class="breadcrumb-item active">View Cities</li>`;
