				...
				<div class="row">
					<div class="col-12">
						<div class="page-title-box d-sm-flex align-items-center justify-content-between">
							<h4 class="mb-sm-0">Add Credential</h4>
						...
						<div class="card">
							<div class="card-header border-0">
								<div class="d-flex align-items-center">
									<h5 class="card-title mb-0 flex-grow-1">Add Credential</h5>
								</div>
							</div>

							<div class="card-body pt-0">
								<form method="post" id="add_credentials">
								<div class="row">
									<div class="col-xxl-3 col-md-4">
										<div>
											<label for="labelInput" class="form-label">Hotel Name</label>
											<select name="cred_hotel_name" id="cred_hotel_name" class="form-control cred_hotel_name">
												<option value=""></option>
...
function GetHotelNameByWebsite(){
	...
	$.ajax({
		...
		success: function (res) {
			...
			if (res.status == 1) {
				$("#cred_hotel_name").select2('destroy').val("").select2({
					placeholder: "Select Hotel Name",
				});
				$('#cred_hotel_name').html(res.html);
			}
...
$(document).on("click", "#save_hotel", function (){
	var formData = new FormData($('#add_credentials')[0]);
	formData.append('customer', $("#users_dropdown").val());

	if(validate_add_credentials_form()){
		$.ajax({
			...
			success: function (res) {
				...
				if (res.status == "error"){
					...
					$("#success_toast").attr('data-toast-text', "Credentials added successfully.");
...
function redirect_listpage(){
	location.href = "<?php echo base_url('review_management/manage_credentials')?>";
}

function validate_add_credentials_form()
{
	$('#add_credentials').validate({
		rules: {
			website: {
				required: true,
			},
