	...
	private function processModeColor($mode, array $cores)
	{
		...
		switch ($mode) {
			case 'rgb':
				...
				else if (isset($cores[2])) {
					return [static::MODE_RGB, $cores[0], $cores[2]];
				}
				else {
					return [static::MODE_RGB, $cores[0]];
				}
			...
			case 'rgba':
				if (isset($cores[1])) {
					return [static::MODE_RGBA, $cores[0], $cores[1], $cores[2], $cores[3] * 100];
						...
						return [static::MODE_RGBA, $cores[0], $cores[2], $cores[3] * 100];
				}
				else if(isset($cores[3])) {
					return [static::MODE_RGBA, $cores[0], $cores[3] * 100];
				}
				else {
					return [static::MODE_RGBA, $cores[0]];
				}
